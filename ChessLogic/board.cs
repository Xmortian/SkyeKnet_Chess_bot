...
{
    // Class representing the chessboard, holding the pieces in an 8x8 grid
    public class board
    {
        // 2D array to store the pieces on the board
        ...
        // Indexer to access pieces by Position (Row and Column)
        ...
        // Static method to create an initial board setup with all pieces in their starting positions
        public static board Initial()
        {
            board board = new board(); // Create a new board
            board.AddStartPieces(); // Set up the starting pieces
            return board; // Return the initialized board
        }

        // Private method to add the starting pieces to the board
        private void AddStartPieces()
        {
            // Place the black pieces in their starting positions
            ...
            // Place the white pieces in their starting positions
            ...
            // Place the pawns on their starting rows (2nd and 7th rows)
            for (int c = 0; c < 8; c++)
            {
                this[1, c] = new Pawn(Player.Black); // Black pawns
                this[6, c] = new Pawn(Player.White); // White pawns
            }
        }

        // Static method to check if a given position is within the bounds of the board
        public static bool IsInside(Position pos)
        {
            return pos.Row >= 0 && pos.Row < 8 && pos.Column >= 0 && pos.Column < 8;
        }

        // Method to check if a given position is empty (no piece at that position)
        public bool IsEmpty(Position pos)
        {
            return this[pos] == null; // If there's no piece at the position, it's empty
        }
        ...
        private static bool IsKingVKing(Counting counting)
        {
            ...
            return counting.TotalCount == 2; // Only two kings
        }
        ...
        public bool CastleRightQS(Player player) 
        {
            return player switch
            {
                Player.White => IsUnmovedKingAndRook(new Position(7, 4), new Position(7, 0)), // White: King (7,4) and Rook (7,0)
                Player.Black => IsUnmovedKingAndRook(new Position(0, 4), new Position(0, 0)), // Black: King (0,4) and Rook (0,0)
                _ => false
            };
        }
